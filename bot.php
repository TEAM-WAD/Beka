...
$pdo->exec("CREATE TABLE IF NOT EXISTS bot_settings (key TEXT PRIMARY KEY, value TEXT)");
...
// جلب آخر update_id تم معالجته لتجنب تكرار الرسائل
$stmt = $pdo->prepare("SELECT value FROM bot_settings WHERE key = 'last_update_id'");
$stmt->execute();
$row = $stmt->fetch(PDO::FETCH_ASSOC);
$offset = $row ? intval($row['value']) + 1 : 0;

// جلب التحديثات الجديدة من تليجرام
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, $website . '/getUpdates?offset=' . $offset . '&timeout=0');
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
$output = curl_exec($ch);
curl_close($ch);
$updatesResponse = json_decode($output, true);

if (!isset($updatesResponse['result']) || empty($updatesResponse['result'])) {
    exit("No new updates.");
}
...
foreach ($updatesResponse['result'] as $update) {
    ...
    // تحديث الأوفست فوراً في قاعدة البيانات
    $stmt = $pdo->prepare("INSERT OR REPLACE INTO bot_settings (key, value) VALUES ('last_update_id', ?)");
    ...
    if($text === "/start" || $text === "main_menu") {
        ...
        if($isUserAdmin) {
            ...
            $adminUsername = "Al_Dulaimi_Admin";
    ...
    if($isUserAdmin) {
        ...
        if($text === 'del_admin_menu') {
            ...
            foreach($admins as $adm) {
                if($adm['user_id'] == $adminId) continue;
        ...
        if(strpos($text, 'type_') === 0 || strpos($text, 'srv_page_') === 0) {
            ...
            $offsetPage = ($page - 1) * $perPage;
            $pageServices = array_slice($filteredServices, $offsetPage, $perPage);
